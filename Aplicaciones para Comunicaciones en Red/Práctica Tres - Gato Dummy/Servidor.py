...
class Turnos(object):
    def __init__(self, start=0):
        self.condicionTurno = threading.Condition()
        self.turnoUno = threading.Condition()
        self.turnoDos = threading.Condition()
        self.turnoTres = threading.Condition()
        self.iniciaJuego = threading.Condition()
        self.numJugadores = 0
        self.conectados = 0
        self.pocision = 1

    def iniJuego(self, conn):
        with self.iniciaJuego:
            if self.numJugadores > self.conectados:
                # response = bytes("\n***Esperando al resto de jugadores***\n", 'ascii')
                # conn.sendall(response)
                # print(response)
                self.iniciaJuego.wait()
            else:
                # response = bytes("\n***Inciando***\n", 'ascii')
                # conn.sendall(response)
                # print(response)
...
def servirPorSiempre(socketTcp, listaconexiones, turnos, bloqueo):
    try:
        # thread_admin = threading.Thread(target=gestion_turnos, args=[turnos])
        # thread_admin.start()
        while True:
...
def recNumJugadores(conn):
    try:
        ...
        while True:
            ...
            if not data:
                break
            print("numero de jugadores : ", repr(data), "")
...
def recibir_datos(conn, listaconexiones, turnos, bloqueo):
    try:
        ...
        turnos.iniJuego(conn)  # Espera a que todos los jugadores se unan

        while True:
            print("Turno: ", turnos.getPosicion())
            turnos.espera(turnoHilo)
            # RECIBE
